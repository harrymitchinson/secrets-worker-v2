...
export default function Panel({ title, children }: PropsWithChildren<Props>) {
  return (
    <>
      <header className="w-full max-w-screen-md sm:rounded-t bg-indigo-700 dark:bg-indigo-700 shadow-md py-8 px-4 sm:p-8 font-bold text-center font-bold text-2xl">
        <h1 className="text-2xl text-indigo-100 dark:text-indigo-100">
          {title}
        </h1>
      </header>
      ...
      <footer className="w-full max-w-screen-md sm:rounded-b bg-gray-200 dark:bg-gray-700 shadow-md py-4 px-4 sm:px-8 text-center">
        ...
        <span className="text-gray-300 dark:text-gray-600">{`|`}</span> View
        source code on{" "}
        <a
